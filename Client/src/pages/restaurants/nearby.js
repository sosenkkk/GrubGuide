...
export default function Products() {
  ...
  const [lat, setLat] = useState(router.query.latitude);
  const [long, setLong] = useState(router.query.longitude);
  ...
  return (
    <>
      <Head>
        <title>GrubGuide - Near by Restaurants</title>
      </Head>
      ...
      <div className="pt-[240px] min-h-screen transition-colors md:pt-[250px] bg-[#f9f9f9] dark:bg-[#171717] p-4 sm:px-8 py-0">
        ...
        <div className="flex-wrap sm:flex-nowrap flex gap-2 w-full">
          ...
          {productsLoaded ? (
            <div className=" w-full  mt-8 gap-4 gap-y-8 productContainerHolder">
              ...
              {restaurants.length == 0 && (
                <div className="flex flex-col gap-2 p-8">
                  <div className="text-3xl font-bold">
                    {" "}
                    No Restraunts found within 10km of your locality.
                  </div>
                  <div className="text-2xl opacity-80 text-green-400 underline">
                    {" "}
                    <Link href="/restaurants">Try Visiting here for more filters.</Link>
                  </div>
                </div>
              )}
            </div>
          ) : (
            <div className="min-h-screen  transition-colors  bg-[#f9f9f9] dark:bg-[#202020] p-4 sm:px-8 py-0 flex w-full items-center justify-around">
